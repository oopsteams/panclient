...
var MultiFileLoader = Base.extend({
	...
	build_main_tasks:function(){
		...
		var task_param_list = query_tasker_list_from_local(this.task.id, (task_param_list)=>{
			ithis.pause();
				...
				if(states.size>0){
					if(t.size() == states.size){
						...
						if(states.size < 2048){
							...
							var task_params = {'id':id_prefix+'_'+pos, 'source_id':t.params.source_id, 'start':new_start, 'end':t.params.end, 'over':0, 'pos':0, 'retry':0, 'loader_id': t.params.loader_id, 'state': 0};
